...
public class SecondKillAction {
  ...
  private final static  String Lua_seconds_kill=" local isExist = redis.call('exists', KEYS[1])             \n" +
      "         if isExist == 1 then\n" +
  ...
  public Result killv1(@PathVariable("id") String id, HttpServletRequest request) {
    ...
    try {
      ...
      if (!b) {
        return Result.successObject("你没有抢到");
      }
      //在redis里库存减1  stock库存
      ...
      if(stock <= 0){
        ...
        return Result.successObject("秒杀产品库存为0");
      }else {
        //创建订单 通过mq进行下订单
    ...
    return Result.successObject(null);
